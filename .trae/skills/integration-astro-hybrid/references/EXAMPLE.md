...
This is an [Astro](https://astro.build/) hybrid rendering example demonstrating PostHog integration with both static and
on-demand rendered pages.

Hybrid mode allows you to have most pages prerendered (static) while opting specific pages into server-side rendering
(SSR) when needed.
...
In Astro 5, `output: 'static'` is the default and supports per-page SSR opt-in. You need an adapter for the SSR pages to
work:
...
<script
  is:inline
  define:vars={{ apiKey: import.meta.env.PUBLIC_POSTHOG_PROJECT_TOKEN, apiHost: import.meta.env.PUBLIC_POSTHOG_HOST }}
>
  !(function (t, e) {
    var o, n, p, r;
    e.__SV ||
...
<PostHogLayout title="Burrito Consideration - Astro PostHog Hybrid Example">
  <div class="container">
    ...
    <div style="text-align: center;">
      <button id="consider-btn" class="btn-burrito"> I have considered the burrito potential </button>
    ...
    <p class="note" style="margin-top: 1rem;">Events are tracked both client-side and server-side for demonstration.</p>
  </div>
</PostHogLayout>
...
<PostHogLayout title="Home - Astro PostHog Hybrid Example">
  <div class="container">
    ...
    <div id="logged-out-view">
      ...
      <p class="note">Note: This is a demo app with server-side tracking. Use any username and password to sign in.</p>
    </div>
  </div>
</PostHogLayout>
...
<PostHogLayout title="Profile - Astro PostHog Hybrid Example">
  <div class="container">
    ...
    <div style="margin-top: 2rem;">
      ...
      <p id="error-feedback" class="success" style="display: none;">Error captured and sent to PostHog!</p>
    </div>
  </div>
</PostHogLayout>
